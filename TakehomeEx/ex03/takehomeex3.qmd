...
### 6.5.3 Path of Snapper Snatcher
...
### 6.5.5 When and Where did the illegal fishing took place?  
...
other_trespassers <- other_trespassers %>%
  mutate (stay_duration = stay_duration/360)
...
```{r}
vessel_movement_robber <- vessel_movement_sf %>%
  filter(target == "snappersnatcher7be")
```
...
ggplot() +
  ...
  labs(title = " Carp Capturer spent time at Himark, Wrasse Beds and Nav C", 
       subtitle = "No illegal activities detected", 
       x = "Longitude", y = "Latitude")
```
