...
```{r}
pacman::p_load(jsonlite, tidygraph, igraph, ggraph, visNetwork, graphlayouts, ggforce, skimr, tidytext, tidyverse, plotly, shiny, DT)
```
...
heatmap_plot <- ggplot(visit_counts, aes(x = month, y = port, fill = visit_count)) +
  ...
  labs(title = "Paackland and Haacklee are the busier ports",
       x = "Month",
       y = "Port",
...
ggplot(transponder, aes(x = area, fill = type)) +
  ...
  labs(title = "Cargo Vessels should not visit Fishing Grounds or Reserves",
       x = "Location Type",
       y = "Count",
       fill = "Vessel Type",
...
boxplot <- ggplot(reserves_data, aes(x = type, y = stay_duration, fill = type)) +
  ...
  labs(title = "Many Instances of Fishing Vessel Overspending Time in Reserves",
       ...
       subtitle = "71 Cargo Vessels and 131 Fishing Vessels passed through Reserves") +
  scale_y_log10() + 
  theme_minimal()
...
![](images/clipboard-2365353441.png)
...
# Plot the monthly delivery
ggplot(filtered_summary, aes(x = month, y = num_deliveries, color = fishtype)) +
  geom_line(size = 1) +  # Line plot for monthly trend
  geom_point(size = 2) +  # Add points for each month
  labs(title = "Deliveries of Sockfish, Salmon, Offidiaa, and Helenaa",
       x = "Month",
       y = "Number of Deliveries",
       subtitle = "Delivery of Salmon peaked in Sep, the rest in Nov",
       color = "Fish Type") +
  theme_minimal() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", limits = as.Date(c("2035-02-01", "2035-11-31"))) +
  geom_text(data = filtered_summary, aes(label = num_deliveries), size = 2, vjust = -1) + 
...
-   Snapper Snatcher - a small 100 ton vessel

-   Roach Robber - a 11700 ton bigger vessel
...
## 6.4 Visualising Vessel Movement Data 
...
ggplot() +
  ...
  geom_point(data = points_data, 
             aes(x = st_coordinates(geometry)[, "X"], 
                 ...
                 size = dwell_time), 
             color = "black", alpha = 0.7) +  # Add points with size based on dwell time
  scale_size_continuous(name = "Dwell Time") +  # Customize size legend
  theme_minimal() +
  labs(title = "Trajectories of albacoreangler47d and Dwell Time", 
       x = "Longitude", y = "Latitude", color = "ID")
```
