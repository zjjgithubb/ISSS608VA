...
region_size <- ggplot(data=condo_subset2324, 
  ...
  annotate("text", x=4.6, y=130, label=format(resalesize24_median, big.mark = ","),
           size=3, color="red")+
  ylim(0,200) +
  labs(title="Unit Area of Condos", y = "Unit Area (sqm)", x = "Planning Region")
...
region_unitprice <- ggplot(data=condo_subset2324, 
  ...
  annotate("text", x=4.6, y=27000, label=format(resaleunit24_median, big.mark = ","),
           size=3, color="red")+
  ylim(0,40000) +
  labs(title="Unit Price ($ sqm) of Condos", y = "Unit Price ($ sqm)", x = "Planning Region")
...
p2 <- resale_unitprice24 + plot_annotation(title = "Both Mean and Median Prices of Resale Transactions are higher in Q1Y24 compared to 2023")
